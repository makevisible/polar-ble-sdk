...
extension PolarBleApiImpl: PolarBleLowLevelApi {
    ...
    // Visible fork: getFileList above drops sizes; the Files Viewer diagnostics tooling
    // needs them, and fetchPftpEntries already computes them.
    func getFileListWithSizes(identifier: String, directoryPath: String, recurseDeep: Bool) async throws -> [(name: String, size: UInt64)] {
        logApiCall("getFileListWithSizes", ("identifier", identifier), ("directoryPath", directoryPath), ("recurseDeep", recurseDeep))
        let session = try serviceClientUtils.sessionFtpClientReady(identifier)
        guard let client = session.fetchGattClient(BlePsFtpClient.PSFTP_SERVICE) as? BlePsFtpClient else {
            throw PolarErrors.serviceNotFound
        }
        ...
        if path.last != "/" { path.insert("/", at: path.endIndex) }
        let condition = { (entry: String) -> Bool in entry.contains(".") || entry == "" }
        return try await fetchPftpEntries(path, client: client, condition: condition, recurseDeep: recurseDeep)
    }
