...
class BDBleApiImpl private constructor(context: Context, features: Set<PolarBleSdkFeature>) : PolarBleApi(features), BlePowerStateChangedCallback {
                ...
                return fetchRecursively(
                    ...
                    .flatMap { groupedEntries ->
                        groupedEntries
                            ...
                            .distinct { entry -> entry.date.time }
                            .onErrorResumeNext { throwable: Throwable ->
                                Flowable.error(handleError(throwable))
                            }
                    }.onErrorResumeNext { throwable: Throwable ->
    ...
    override fun getTrainingSession(
        ...
        return Single.create { emitter ->
            try {
                ...
                BleLogger.e(TAG, "Failed to initiate training session read: $error")
                emitter.onError(error)
            }
        }
    }
    ...
    override fun deleteStoredDeviceDataOld(identifier: String, dataType: PolarStoredDataType, until: LocalDate?): Flowable<ConcurrentLinkedQueue<String>> {
            ...
            return Observable.fromIterable(dates)
                .flatMap { date ->
                    ...
                    return@flatMap client.request(builder.build().toByteArray())
                        .toObservable()
                        .onErrorResumeNext { throwable: Throwable ->
                            if (throwable.message == "NO_SUCH_FILE_OR_DIRECTORY") {
